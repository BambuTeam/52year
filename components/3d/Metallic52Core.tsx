...
// GLSL 3D Simplex Noise for Procedural Organic Liquid Shader
...
function ProceduralLiquidFluid({ powerUpTimestamp }: { powerUpTimestamp?: number }) {
  ...
  const handleBeforeCompile = (shader: any) => {
    ...
    shader.uniforms.uDisplacement = uniformsRef.current.uDisplacement;
    shader.uniforms.uFrequency = uniformsRef.current.uFrequency;
  ...
  useFrame((state, delta) => {
    ...
    uniformsRef.current.uFrequency.value = THREE.MathUtils.damp(
      uniformsRef.current.uFrequency.value,
      targetFrequency,
      6,
      delta
    );
...
export function Metallic52Core({ powerUpTimestamp, introProgress = 1 }: Metallic52CoreProps) {
  ...
  return (
    <group ref={coreGroup} position={[0, 0, 0]}>
      ...
      {/* 2. High-End Procedural WebGL Organic Liquid Shader Blob (Replacing Static fluido.glb) */}
      <ProceduralLiquidFluid powerUpTimestamp={powerUpTimestamp} />
